...
export const useAuthStore = create<AuthState>()((set, get) => ({
  ...
  logout: async () => {
    try {
      const { token } = get()
      if (token) {
        // Intentar llamar a la API para cerrar sesión
        // Si falla (401, token inválido, etc.), continuar de todos modos
        await api.post('/logout').catch(() => {
          // Silenciosamente ignorar errores de logout
          // El token se limpiará de todos modos
        })
      }
    } catch (err) {
      // Ignorar cualquier error
    } finally {
      // Siempre limpiar estado, independientemente de errores
      localStorage.removeItem('token')
      set({
        user: null,
        token: null,
