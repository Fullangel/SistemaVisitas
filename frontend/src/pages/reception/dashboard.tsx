...
export default function ReceptionDashboard() {
  ...
  const [stats] = useState<ReceptionStats>({
    pending_checkins: 8,
    active_visits: 12,
    completed_today: 24,
    pending_checkouts: 3,
  ...
  const handleViewVisit = (visit: Visit) => {
    console.log('Ver detalles de visita:', visit)
  ...
  const handlePrintBadge = (visit: Visit) => {
    console.log('Imprimir gafete:', visit)
  }
  ...
  const receptionSections = [
    {
      ...
      color: 'green',
      actions: [
        { name: 'Nueva Visita', icon: PersonAddRegular, action: handleNewVisit, color: 'green' },
        { name: 'Check-in Rápido', icon: QrCodeRegular, action: () => console.log('Check-in QR'), color: 'blue' },
        { name: 'Registrar Grupo', icon: PeopleRegular, action: () => console.log('Registrar grupo'), color: 'purple' },
        { name: 'Verificar ID', icon: ContactCardRegular, action: () => console.log('Verificar ID'), color: 'orange' }
      ]
    },
    {
      id: 'checkout',
      ...
      color: 'red',
      actions: [
        { name: 'Check-out Individual', icon: SignOutRegular, action: () => console.log('Check-out individual'), color: 'red' },
        { name: 'Check-out Grupal', icon: PeopleRegular, action: () => console.log('Check-out grupal'), color: 'rose' },
        { name: 'Generar Comprobante', icon: DocumentTextRegular, action: () => console.log('Generar comprobante'), color: 'indigo' },
        { name: 'Imprimir Reporte', icon: PrintRegular, action: () => console.log('Imprimir reporte'), color: 'slate' }
      ]
    },
    {
      id: 'management',
      ...
      color: 'blue',
      actions: [
        { name: 'Buscar Visita', icon: SearchRegular, action: () => console.log('Buscar visita'), color: 'blue' },
        { name: 'Editar Visita', icon: DocumentTextRegular, action: () => console.log('Editar visita'), color: 'cyan' },
        { name: 'Cancelar Visita', icon: DismissRegular, action: () => console.log('Cancelar visita'), color: 'red' },
        { name: 'Reprogramar', icon: CalendarRegular, action: () => console.log('Reprogramar'), color: 'amber' }
      ]
    },
    {
      id: 'communication',
      ...
      color: 'purple',
      actions: [
        { name: 'Notificar Llegada', icon: PhoneRegular, action: () => console.log('Notificar llegada'), color: 'blue' },
        { name: 'Enviar Correo', icon: MailRegular, action: () => console.log('Enviar correo'), color: 'green' },
        { name: 'Compartir Info', icon: ShareRegular, action: () => console.log('Compartir info'), color: 'purple' },
        { name: 'Alertas', icon: WarningRegular, action: () => console.log('Alertas'), color: 'red' }
      ]
    }
  ]
            ...
            <div className="flex items-center space-x-4">
              <div className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(stats.alerts_count)}`}>
                <span className="flex items-center">
                  <WarningRegular className="w-3 h-3 mr-1" />
                  {stats.alerts_count} Alertas
                </span>
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400">
                Recepcionista: {user ? `${user.first_name} ${user.last_name}` : 'Recepción'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-8">
          <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <PersonAddRegular className="h-4 w-4 text-yellow-600" />
            </CardHeader>
            <CardContent>
          ...
          <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <SignOutRegular className="h-4 w-4 text-orange-600" />
            </CardHeader>
            <CardContent>
          ...
          <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CheckmarkCircleRegular className="h-4 w-4 text-blue-600" />
            </CardHeader>
            <CardContent>
          ...
          <Card className="backdrop-blur-sm bg-white/80 dark:bg-gray-800/80">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <ClockRegular className="h-4 w-4 text-purple-600" />
            </CardHeader>
            <CardContent>
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="relative">
                  <SearchRegular className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <Input
                    type="text"
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
                ...
                <div key={visit.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border">
                  <div className="flex items-center space-x-4">
                    <div className="flex-shrink-0">
                      <QrCodeRegular className="h-6 w-6 text-gray-400" />
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center space-x-4 mt-1">
                        <div className="flex items-center text-xs text-gray-500">
                          <ContactCardRegular className="w-3 h-3 mr-1" />
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {visit.estado === 'pendiente' && (
                        ...
                        >
                          <CheckmarkCircleRegular className="h-4 w-4 mr-1" />
                          Check-in
                        </Button>
                      )}
                      {visit.estado === 'en_curso' && (
                        ...
                        >
                          <SignOutRegular className="h-4 w-4 mr-1" />
                          Check-out
                        </Button>
                      )}
                      <div className="flex items-center space-x-2">
                        <Button variant="ghost" size="sm" onClick={() => handleViewVisit(visit)}>
                          <ContactCardRegular className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleEditVisit(visit)}>
                          <EditRegular className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handlePrintBadge(visit)}>
                          <PrintRegular className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>
                  </div>
              ...
              {filteredVisits.length === 0 && (
                <div className="text-center py-8">
                  <PersonRegular className="h-12 w-12 text-gray-400 mx-auto mb-4" />
